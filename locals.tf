locals {
  ...
  kubelet_arg                 = concat(["cloud-provider=external", "volume-plugin-dir=/var/lib/kubelet/volumeplugins", "allowed-unsafe-sysctls=net.core.somaxconn,net.ipv4.tcp_max_syn_backlog"], var.k3s_kubelet_config != "" ? ["config=/etc/rancher/k3s/kubelet-config.yaml"] : [])
